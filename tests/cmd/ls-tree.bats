...
assert_matches_git_ls_tree() {
    ...
    # qgit defaults to HEAD; git ls-tree requires an explicit tree-ish.
    if [ $# -eq 0 ] || [[ "$last" == -* ]]; then
        git_args=("$@" HEAD)
    fi
...
@test "qgit ls-tree: default HEAD matches git" {
    setup_single_file_commit >/dev/null
    assert_matches_git_ls_tree
}

@test "qgit ls-tree: default HEAD on linear history matches git" {
    setup_linear_history 4 >/dev/null
    assert_matches_git_ls_tree
}
...
@test "qgit ls-tree: default HEAD follows non-default branch" {
    setup_multi_branch_history
