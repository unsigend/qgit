                       ...
                       qgit_branch_t branch_type)
{
    ...
    if (branch_type & QGIT_BRANCH_LOCAL) {
        if (snprintf(bname, PATH_MAX, "refs/heads/%s", branch_name) >=
            PATH_MAX) {
            errno = ENAMETOOLONG;
            return -1;
        }
    } else if (branch_type & QGIT_BRANCH_REMOTE) {
        if (snprintf(bname, PATH_MAX, "refs/remotes/%s", branch_name) >=
            PATH_MAX) {
            errno = ENAMETOOLONG;
            return -1;
        }
    ...
    if (qgit_reference_lookup(&ref, repo, bname) == -1)
        return -1;

    if (qgit_reference_delete(ref) == -1) {
