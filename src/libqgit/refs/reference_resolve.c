...
int qgit_reference_resolve(qgit_reference **out, const qgit_reference *ref)
{
    ...
    *out = NULL;
    ...
    if (ref->type == QGIT_REF_DIRECT) /* already a direct reference */
    {
        *out = calloc(1, sizeof(qgit_reference));
        if (!*out)
            return -1;
        (*out)->type = QGIT_REF_DIRECT;
        (*out)->owner = ref->owner;
        ...
        qgit_oid_cpy(&(*out)->target.oid, &ref->target.oid);

        return 0;
    } else if (ref->type == QGIT_REF_SYMBOLIC) /* symbolic reference */
    {
        qgit_reference *cur;
