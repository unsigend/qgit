...
int cmd_init(int argc, char **argv)
{
    ...
    struct argparse_opt opts[] = {
        ...
        OPT_STR('b', "initial-branch",
                "Use <branch-name> for the initial branch ", &branch,
                OPT_REQUIRED),
        OPT_END(),
    };
    ...
    struct argparse_desc desc = {
        .prog = "qgit init",
        .desc =
            "Create an empty qgit repository or reinitialize an existing one ",
        .usages = usages,
    ...
    if (qgit_repository_init(&repository, abspath, branch) == -1)
        die_errno();
