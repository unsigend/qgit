...
QGIT_EXTERN(int) qgit_repository_discover(char *repo_path, size_t size,
                                           const char *start_path);

/* Initialize a new repository at path with the given initial branch name. */
QGIT_EXTERN(int) qgit_repository_init(qgit_repository **repo, const char *path,
                                       const char *branch);
...
QGIT_EXTERN(int) qgit_repository_head(qgit_reference **head, qgit_repository *repo);
...
/* Return 1 if HEAD points to a branch that has no commits (unborn), 0 otherwise. */
...
QGIT_EXTERN(int) qgit_repository_config(qgit_config **out, qgit_repository *repo);
